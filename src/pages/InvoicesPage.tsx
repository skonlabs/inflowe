...
export default function InvoicesPage() {
  ...
  const filtered = demoInvoices.filter(inv => {
    ...
    if (states && states.length > 0 && !states.includes(inv.state)) return false;
  ...
  return (
    <div className="px-4 py-6 space-y-4">
      <ScrollReveal>
        ...
        <p className="text-sm text-muted-foreground mt-1">{demoInvoices.length} total · {demoInvoices.filter(i => i.state === 'overdue').length} overdue</p>
      </ScrollReveal>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 -mx-4 px-4 scrollbar-hide">
          {filters.map(f => (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-colors active:scale-95 ${
                activeFilter === f
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <div className="text-right shrink-0 ml-3">
                  ...
                  <span className={`inline-block text-[10px] px-2 py-0.5 rounded-full font-medium mt-1 ${getStateClass(inv.state)}`}>
                    {getStateLabel(inv.state)}
                    {inv.daysOverdue > 0 && inv.state === 'overdue' && ` · ${inv.daysOverdue}d`}
                  </span>
                </div>
              </div>
              {inv.nextActionAt && (
